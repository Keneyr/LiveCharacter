...
namespace OpenPose {
    ...
    public class OPWrapper : MonoBehaviour {
        ...
        {
            //if (_editmode) state = OPState.Ready;
        ...
        public static void CusOPRun(bool _editmode = false) {
            if (_editmode) state = OPState.Ready;
            if (state == OPState.Ready) {
                // Start OP thread
                state = OPState.Running;
        ...
        public static bool OPGetOutput(out OPDatum data)
        {
            ...
            {
                Debug.Log("dataBuffer.Count>0");
                data = dataBuffer.Dequeue(); //出队
                return true;
            }
            else
            {
        ...
        // Get NEXT output frame. Note that frame is possible to be omitted if buffer size overflow. 
        public static void CusOPGetOutput(out OPDatum data){
            //尝试不用多线程，都在一个主线程执行
            OPExecuteThread();
        ...
        public static void CusOnDestroy()
        {
